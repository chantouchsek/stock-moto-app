<template>
    <f7-page>
        <f7-navbar title="Edit Make" back-link="Back" sliding></f7-navbar>
        ...
        <f7-block>
            <f7-row>
                <f7-col>
                    <f7-button fill @click.native="updateMake" big outline round>
                        <i class="f7-icons size-16">edit</i> Edit
                    </f7-button>
                </f7-col>
                <f7-col>
                    <f7-button fill color="red" big outline round @click.native="destroyMake(form)">
...
<script>
  import MakeProxy from '@/proxies/MakeProxy'
  import MakeTransformer from '@/transformers/MakeTransformer'

  const proxy = new MakeProxy()
  ...
  export default {
    ...
    methods: {
      ...
      /**
       * Method used to fetch an make.
       *
       * @param {number | string} id The id of the make.
       */
      fetchMake (id) {
        proxy.find(id)
          .then((data) => {
            this.form = MakeTransformer.fetch(data)
          })
      },
      /**
       * Method to edit an existing make.
       * It'll dispatch the update action on the make module.
       */
      updateMake () {
        const self = this
        self.$f7.preloader.show()
        self.$store.dispatch('make/update', self.form)
      },
      /**
       * Delete the resource
       */
     ...
     */
    mounted () {
      this.fetchMake(this.$f7route.params.uuid)
    },
    watch: {
